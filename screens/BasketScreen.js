    ...
    return ( 
        <SafeAreaView>
            <View className="flex-1 bg-white">
                ...
                <View className="flex-row items-center space-x-4 bg-white
                px-4 py-3 my-5
                ">
                    <Image 
                        source={{
                    ...
                    <TouchableOpacity>
                        <Text className="color-[#00CCBB]">Change</Text>
                    </TouchableOpacity>
                </View>

                    {/* 
                    we have structure with object with keys and array inside those keys
                    */}
                <ScrollView className="divide-y divide-gray-200">
                    
                    {Object.entries(groupedItemsInBasket).map(([key, items]) => (
                        <View key={key}
                        className="flex-row items-center space-x-3 py-2 px-5"
                        >
                            ...
                            <Text>{items[0]?.price} GBP</Text>
                            <TouchableOpacity onPress={() => dispatch(removeFromBasket({id: key}))}>
                                <Text className="color-[#00CCBB] text-xs">Remove</Text>
                            </TouchableOpacity>
                        </View>
                    ))}
                </ScrollView>
                ...
                {/* sub total */}
                <View className="p-4 mt-5 space-y-5 bg-white">
                    ...
                    <View className="flex-row justify-between">
                        ...
                        <Text className="text-gray-800 font-bold">{basketTotal + 5.99} GBP</Text>
                    </View>
